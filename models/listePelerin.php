...
require_once('../controllers/database_connexion.php');
    ...
    $pelerin0=$base->prepare("SELECT * FROM pelerin where statut='Actif'");
    $pelerin0->execute();
    ...
    for($i=0; $i<count($pelerin); $i++){
        ...
        <td> 
        <a href='../views/modifPelerin.php?idPelerin=$idPelerin' class='text-warning me-2 editbtn' title='Modifier'><i class='fas fa-edit'></i></a> 
        ...
        <a href='../models/supprimePelerin.php?idPelerin=$idPelerin' class='text-danger me-2 deletebtn' title='Archiver'><i class='fas fa-trash'></i></a>
        </td></tr>";
